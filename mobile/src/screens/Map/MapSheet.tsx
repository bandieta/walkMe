    ...
    {SEGMENTS.map((o) => {
      ...
      return (
        <Pressable key={o.key} onPress={() => onChange(o.key)} style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
          ...
          <Text style={{ fontSize: 13, color: on ? Colors.primary : Colors.textPrimary }}>{o.label}</Text>
        </Pressable>
      );
    })}
  </View>
...
export const MapSheet: React.FC<{
  ...
  return (
    ...
    >
      ...
      />
      <View {...pan.panHandlers} accessibilityRole="button" accessibilityLabel={open ? 'Collapse' : 'Expand'} style={{ height: 26, alignItems: 'center', justifyContent: 'center' }}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingTop: 6, paddingHorizontal: 16, paddingBottom: 110 }} showsVerticalScrollIndicator={false}>
        ...
        {items.length === 0 && (
          loading ? (
            ...
            <Text style={{ paddingVertical: 28, fontSize: 13, color: Ramp.neutral[500] }}>Nothing matches. Try another filter or search.</Text>
          )
        )}
      </ScrollView>
    </Animated.View>
