...
export const ProfileScreen: React.FC<{ navigation: any; route?: any }> = ({
  ...
  const upcoming =
    walks.filter((w) => w.status !== 'ended' && (w.participantIds ?? []).includes(user?.id))
      .length + events.filter((e) => e.isJoined && e.status !== 'ended').length;
  const ageGroupOf = (d: any) => ageGroupFromAge(t, d.age, d.ageGroup);

  const prefRows: { key: keyof typeof prefs; label: string; desc: string }[] = [
    {
      key: 'loc',
      label: t('profile.preferences.shareLocation.label'),
      desc: t('profile.preferences.shareLocation.desc'),
    },
  ];
  const menuRows: { icon: IconName; label: string; meta: string; go: () => void }[] = [
    {
      icon: 'path',
      label: t('profile.account.walksEvents.label'),
      meta: t('profile.account.walksEvents.upcoming', { count: upcoming }),
      go: () => navigation.navigate('MyWalks'),
    },
    {
      icon: 'sliders-horizontal',
      label: t('profile.account.rhythm'),
      meta: radiusLabel,
      go: () => navigation.navigate('Rhythm'),
    },
    {
      icon: 'globe',
      label: t('profile.account.language'),
      meta: '',
      go: () => navigation.navigate('Language'),
    },
    {
      icon: 'bell',
      label: t('profile.account.notifications'),
      meta: '',
      go: () => navigation.navigate('NotificationSettings'),
    },
    {
      icon: 'shield-check',
      label: t('profile.account.privacy.label'),
      meta: '',
      go: () => navigation.navigate('PrivacySafety'),
    },
    {
      icon: 'question',
      label: t('profile.account.help.label'),
      meta: '',
      go: () =>
        Alert.alert(t('profile.account.help.alertTitle'), t('profile.account.help.alertBody')),
    },
  ];
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.identity}>
          ...
          <View style={{ flex: 1, minWidth: 0, rowGap: 2 }}>
            <Text style={cssText(21, { fontWeight: '500' })} numberOfLines={1}>
              {user?.displayName}
            </Text>
            {!!user?.location && (
              <View style={styles.locRow}>
                ...
                <Text
                  style={cssText(13, { color: Ramp.neutral[400], marginLeft: 4, flexShrink: 1 })}
                  numberOfLines={1}
                >
                  {user.location}
                </Text>
        ...
        <View style={styles.stats}>
          {[
            ...
            <View key={label}>
              ...
              <Text numberOfLines={1} style={cssText(12, { color: Ramp.neutral[500] })}>
                {label}
              </Text>
            </View>
          ))}
        ...
        <View>
          <View style={styles.sectionHead}>
            <Text style={cssText(15, { fontWeight: '500' })}>
              {isShelter ? t('profile.dogs.shelterTitle') : t('profile.dogs.title')}
            </Text>
            <Pressable onPress={() => navigation.navigate('MyDogs')} style={{ paddingVertical: 6 }}>
              <Text style={cssText(12, { color: Ramp.accent[300] })}>
                {t('profile.dogs.manage')}
              </Text>
            </Pressable>
          </View>
          <ScrollView
          ...
          >
              ...
              <View key={d.id} style={styles.dogCard}>
                {d.photoUrl ? (
                  <Image
                    ...
                    style={{ height: 84, borderRadius: 8, backgroundColor: '#1f2130' }}
                  />
                ) : (
                  <Placeholder
                    label="dog photo"
                    style={{ height: 84, borderRadius: 8 }}
                    padding={6}
                  />
                )}
                <View style={{ marginTop: 8 }}>
                  <Text style={cssText(14, { fontWeight: '500' })} numberOfLines={1}>
                    {d.name}
                  </Text>
            ...
            <Pressable onPress={() => navigation.navigate('AddDog')} style={styles.addDog}>
              ...
              <Text
                numberOfLines={1}
                style={cssText(12, { color: Ramp.neutral[400], marginTop: 6 })}
              >
                {t('profile.dogs.addDog')}
              </Text>
            </Pressable>
          </ScrollView>
        </View>
          ...
          <View>
            ...
            >
              {connectedShelterDogs.map((r) => (
                <Pressable
                  key={r.id}
                  onPress={() => navigation.navigate('DogRequestChat', { requestId: r.id })}
                  style={styles.dogCard}
                >
                  <View>
                    {r.dog.photoUrl ? (
                      <Image
                        source={{ uri: resolveMediaUrl(r.dog.photoUrl) }}
                        style={{ height: 84, borderRadius: 8, backgroundColor: '#1f2130' }}
                      />
                    ) : (
                      <Placeholder
        ...
        <View>
          ...
          {prefRows.map((p) => (
            <View key={p.key} style={styles.prefRow}>
              ...
              <Toggle
                value={prefs[p.key]}
                onValueChange={(v) => setPrefs((s) => ({ ...s, [p.key]: v }))}
        ...
        <View>
          ...
          {menuRows.map((m) => (
            <Pressable key={m.label} onPress={m.go} style={styles.menuRow}>
              ...
              <Text numberOfLines={1} style={cssText(14, { flex: 1, marginLeft: 12 })}>
                {m.label}
              </Text>
              {!!m.meta && (
                <Text
                  numberOfLines={1}
                  style={cssText(12, { color: Ramp.neutral[500], marginRight: 12, maxWidth: 100 })}
                >
                  {m.meta}
                </Text>
              )}
          ...
          <Pressable onPress={() => setSignOutOpen(true)} style={styles.menuRow}>
            ...
            <Text style={cssText(14, { marginLeft: 12, color: Ramp.neutral[300] })}>
